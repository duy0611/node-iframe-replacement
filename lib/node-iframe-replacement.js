    ...
    return new Promise(function(resolve, reject) {
        ...
        if (isUrl(sourceUrl) && cache.get(sourceUrl)) {
            ...
            return request(params).then(function(html) {
                ...
                // $('head').append('<script src="https://code.jquery.com/jquery-3.2.1.min.js" type="text/javascript"></script>');
                // $('head').append('<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.min.js" type="text/javascript"></script>');
                // $('head').append('<script src="https://cdnjs.cloudflare.com/ajax/libs/jqueryui-touch-punch/0.2.3/jquery.ui.touch-punch.min.js" type="text/javascript"></script>');

				var enableHttps = process.env.ENABLE_HTTPS == 'TRUE' ? true : false;
				
				// var customScript = 'for(var anchors=document.getElementsByTagName("a"),i=0;i<anchors.length;i++){anchors[i].href="http://localhost:12345/proxy?url="+anchors[i].href}';
				$('body').attr('onload', 'tagonFadeOutPreload(' + '"' + process.env.SERVER_NAME + '", ' + '"' + process.env.SERVER_PORT + '", ' + '"' + process.env.TARGET_SERVER_URL + '", ' + enableHttps + ')');
