    ...
    return new Promise(function(resolve, reject) {
        ...
        if (isUrl(sourceUrl) && cache.get(sourceUrl)) {
            ...
            return request(params).then(function(html) {
				...
				$('body').attr('onload', 'tagonFadeInPreload(' + '"' + process.env.TARGET_SERVER_URL + '"' + ')');
				...
				$('body').append('<script type="text/javascript">tagonJquery(document).ready(function(){console.log(tagonJquery().jquery); tagonJqueryLoaded();});</script>');
				...
				if(config.jsConfigMap[shopUrl]) {
					console.log(config.jsConfigMap[shopUrl].externalScripts);
				}		
